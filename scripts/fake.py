...
if __name__ == '__main__':
    ...
    for _ in range(10):
        user = OAUser.objects.create(username=fake.romanized_name(),realname=fake.name(),password='111111')
    ...
    for _ in range(100):
        ...
        pay = Pay.objects.create(name=fake.sentence(),
                                 businessentity='管委会',
                                 paydate=fake.date_time_between(start_date='-1y',end_date="-30d"),
    ...
    for _ in range(10):
        project = Project.objects.create(name=fake.sentence(),
                                         businessentity='管委会',
                                         task_state = TAKESTATE[random.randint(0,len(TAKESTATE)-1)],
                                         transactor=fake.name(),
